...
class CustomMCPClient:
    ...
    async def _send_request(self, method: str, params: Optional[dict[str, Any]] = None) -> dict[str, Any]:
        ...
        response = await self.http_session.post(self.server_url, json=request_data, headers=headers)
        ...
        if response.status == 202:
            return {}  # Successful notification, no content expected
    ...
    async def connect(self) -> None:
        ...
        print("Connecting to MCP server(1)...")
        ...
        try:
            ...
            print("MCP session initialized successfully. Server capabilities:")
    ...
    async def _send_notification(self, method: str) -> None:
        ...
        if not self.session_id and response.headers.get(MCP_SESSION_ID_HEADER):
            self.session_id = response.headers[MCP_SESSION_ID_HEADER]
        print(f"Session ID Initialized: {self.session_id}")
