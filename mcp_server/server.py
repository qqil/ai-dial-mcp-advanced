...
def _validate_accept_header(accept_header: Optional[str]) -> bool:
    ...
    if not accept_header or accept_header.strip() == "" or accept_header == None:
        return False
    
    accept_types = [a.strip().lower() for a in accept_header.split(",")]
    has_json = any("application/json" in a for a in accept_types)
    has_sse = any("text/event-stream" in a for a in accept_types)
...
async def handle_mcp_request(
    ...
    if request.method == "initialize":
        ...
        if session_id:
            ...
            error_response = MCPResponse(
                id="server-error",
                error=ErrorResponse(code=-32603, message="Failed to initialize session")
            )
            return Response(
                status_code=500,
                content=error_response.model_dump_json(),
